...
const cabinPriority = ['Economy', 'Premium Economy', 'Business', 'First'];

const cabinAvailabilityByAircraft = {
  a320neo: ['Economy', 'Premium Economy', 'Business'],
  a321xlr: ['Economy', 'Premium Economy', 'Business'],
  b787: ['Economy', 'Premium Economy', 'Business'],
  a350xwb: ['Economy', 'Premium Economy', 'Business', 'First'],
  b777x: ['Economy', 'Premium Economy', 'Business', 'First'],
};
...
const seededFactor = (seed, min, max) => {
  const normalized = (hashNumber(seed) % 10000) / 10000;
  return min + normalized * (max - min);
};

const deterministicPrice = ({ distanceKm, cabin, demandProfile, isNightFlight, seed }) => {
  ...
  const marketSwing = seededFactor(seed, 0.86, 1.24);
  return Math.round(baseEconomy * cabinFactor * nightFactor * marketSwing);
};

const resolveCabinForAircraft = (requestedCabin, aircraftSlug) => {
  const availableCabins = cabinAvailabilityByAircraft[aircraftSlug] || ['Economy'];
  if (availableCabins.includes(requestedCabin)) return requestedCabin;

  const requestedRank = cabinPriority.indexOf(requestedCabin);
  for (let rank = requestedRank; rank >= 0; rank -= 1) {
    const fallback = cabinPriority[rank];
    if (availableCabins.includes(fallback)) return fallback;
  }

  return availableCabins[0];
};

const buildCabinSummary = (segmentCabins) => {
  const unique = Array.from(new Set(segmentCabins));
  return unique.join(' / ');
};
...
const buildSegmentSummary = ({ itineraryCodes, optionIndex, preferredAircraft, requestedCabin }) => {
  ...
  return baseSegments.map((segment, index) => {
    ...
    const offeredCabinClass = resolveCabinForAircraft(requestedCabin, segmentAircraftSlug);
...
const priceAdultBySegments = ({ segments, isNightFlight, pricingSeed }) => {
  return segments.reduce((total, segment, index) => {
    ...
    const segmentSeed = `${pricingSeed}|${segment.fromCode}-${segment.toCode}|${segment.offeredCabinClass}|${index}`;
    const segmentPrice = deterministicPrice({
      distanceKm: segment.distanceKm,
      cabin: segment.offeredCabinClass,
      demandProfile: segmentDemand,
      isNightFlight,
      seed: segmentSeed,
    });

    // Connection segments usually carry slightly lower prorated fares.
    const connectionFactor = index > 0 ? 0.94 : 1;
    return total + Math.round(segmentPrice * connectionFactor);
  }, 0);
};
...
const generateLegFlights = ({
  ...
  return times.map((departureTime, index) => {
    ...
    const offeredCabinSummary = buildCabinSummary(segments.map((segment) => segment.offeredCabinClass));
    ...
    const adultPrice = priceAdultBySegments({
      segments,
      isNightFlight: Number(departureTime.split(':')[0]) >= 20,
      pricingSeed: `${itineraryCodes.join('-')}|${date}|${cabin}|${tripType}|${leg}|${index}`,
    });
