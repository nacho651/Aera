...
export const formatDayShift = (departure, arrival) => {
  const midnightDeparture = new Date(departure);
  midnightDeparture.setHours(0, 0, 0, 0);

  const midnightArrival = new Date(arrival);
  midnightArrival.setHours(0, 0, 0, 0);

  const dayDiff = Math.round((midnightArrival.getTime() - midnightDeparture.getTime()) / 86400000);
  if (dayDiff <= 0) return '';
  return `+${dayDiff}d`;
};
...
const buildSegmentSummary = (itineraryCodes) =>
  itineraryCodes.slice(0, -1).map((fromCode, index) => {
...
const generateLegFlights = ({
  ...
  return times.map((departureTime, index) => {
    ...
    const depDate = parseDateTime(date, departureTime);
    const tripMinutes = computeTripMinutes(routeDistance, itineraryCodes);
    const arrDate = addMinutes(depDate, tripMinutes);
    ...
    const adultPrice = randomPrice({
      ...
      cabin,
      demandProfile,
      isNightFlight: depDate.getHours() >= 20,
    });
    ...
    return {
      ...
      segments: buildSegmentSummary(itineraryCodes),
      distanceKm: routeDistance,
      departureTime: formatClock(depDate),
      arrivalTime: formatClock(arrDate),
      dayShift: formatDayShift(depDate, arrDate),
